...
func buildSelector(cr *argoproj.ArgoCD) *metav1.LabelSelector {
	return &metav1.LabelSelector{
		MatchLabels: buildLabelsForAgentPrincipal(cr.Name),
	}
}
...
func updateDeploymentIfChanged(compName, saName string, cr *argoproj.ArgoCD, deployment *appsv1.Deployment) (*appsv1.Deployment, bool) {
	...
	if !reflect.DeepEqual(deployment.Spec.Selector, buildSelector(cr)) {
	...
	if !reflect.DeepEqual(deployment.Spec.Template.Spec.Containers[0].Args, buildArgs()) {
		log.Info("deployment container args is being updated")
		changed = true
		deployment.Spec.Template.Spec.Containers[0].Args = buildArgs()
	}
