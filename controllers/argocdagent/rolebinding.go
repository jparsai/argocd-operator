...
func ReconcilePrincipalClusterRoleBinding(client client.Client, compName string, sa *corev1.ServiceAccount, cr *argoproj.ArgoCD, scheme *runtime.Scheme) error {
	...
	//if err := controllerutil.SetControllerReference(cr, newClusterRoleBinding, scheme); err != nil {
	//	return fmt.Errorf("failed to set ArgoCD CR %s as owner for clusterRoleBinding %s: %w", cr.Name, newClusterRoleBinding.Name, err)
	//}
