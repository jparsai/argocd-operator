...
var _ = Describe("GitOps Operator Sequential E2E Tests", func() {
	...
	const (
		...
		principalVerifyResourcesMsg   = "Verify expected resources are created for principal pod"
	)
	...
	Context("1-051_validate_argocd_agent_principal", func() {
		...
		BeforeEach(func() {
			...
			secretNames = []string{
				JWTSecretName,
				principalTLSSecretName,
				rootCASecretName,
				resourceProxyTLSSecretName,
			...
			expectedEnvVariables = map[string]string{
				...
				argocdagent.EnvArgoCDPrincipalTLSSecretName:             principalTLSSecretName,
				argocdagent.EnvArgoCDPrincipalTLSServerRootCASecretName: rootCASecretName,
				argocdagent.EnvArgoCDPrincipalResourceProxySecretName:   resourceProxyTLSSecretName,
				argocdagent.EnvArgoCDPrincipalResourceProxyCaSecretName: rootCASecretName,
				argocdagent.EnvArgoCDPrincipalJwtSecretName:             JWTSecretName,
			}
		...
		createRequiredSecrets := func(namespace *corev1.Namespace, additionalPrincipalSANs ...string) {
			Expect(len(secretNames)).To(BeNumerically(">=", 4))
			...
			agentFixture.CreateRequiredSecrets(agentFixture.PrincipalSecretsConfig{
				...
				JWTSecretName:              secretNames[0],
				PrincipalTLSSecretName:     secretNames[1],
				RootCASecretName:           secretNames[2],
				ResourceProxyTLSSecretName: secretNames[3],
		...
		It("should create argocd agent principal resources, but pod should fail to start as image does not exist", func() {
			...
			By(principalVerifyResourcesMsg)
